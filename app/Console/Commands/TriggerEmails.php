...
class TriggerEmails extends Command
{
    ...
    protected function getMailsInfo ($ids)
    {
        return DB::table('client_services')
                ...
                ->select('clients.name as client_name', 'clients.email','clients.phone_number','client_services.balance','client_services.required_money','services.title as service_name','services.email_template')
                ->get();
    ...
    protected function sendMails ($mails_info)
    {
        ...
        foreach ($mails_info as $data){
            ...
            $send_time=$send_time->addMinutes($late_time);
        }
